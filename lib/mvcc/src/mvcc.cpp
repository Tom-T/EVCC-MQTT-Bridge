/*
  mvcc.cpp - Library for Interacting with MVCC charge controller.
  Created with laziness by copying a similar'ish project https://github.com/Tom-T/MPP-MQTT-Bridge
...
mvcc::mvcc(SoftwareSerial &InverterSerial) : _InverterSerial(InverterSerial)
{
  _InverterSerial.begin(2400);
}
// Are we waiting on a command to finish?
bool mvcc::WaitingForCommand()
{
  if (_pendingCommand == "")
  {
    return false;
...
void mvcc::SetAutomaticRetry(int new_Limit)
{
  if (new_Limit >= 0)
...
void mvcc::_SendCommand(String command)
{
...
void mvcc::_ParseResponse(String command)
{
...
uint16_t mvcc::_Calculate_CRC(uint8_t *pin, uint8_t len)
{
  uint16_t crc;
  uint8_t da;
  uint8_t *ptr;
